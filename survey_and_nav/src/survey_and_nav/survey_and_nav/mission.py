...
class SurveyNode(Node):
    def __init__(self):
        ...
        # No need to have a separate z subscriber and callback as EKF is using z from optical flow itself as of now
        ...
        self.safe_spots_sub = self.create_subscription(
            PoseArray,
            '/safe_spots',
        ...
        # wait for param_cli?? not working
        self.get_logger().info('all mavros services are available.')
        self.get_logger().info('..................................')

        # self.set_param('WPNAV_SPEED', 50)
    ...
    def safe_spots_callback(self, msg):
        ...
        # sorts the safe spots in descending order of their x position
        self.safe_spots.sort(key=lambda spot: spot.position.x, reverse=True)
    ...
    # take_snapshot() captures point-cloud data and saves it as zed_cloud.pcd

    def take_snapshot(self, msg: PointCloud2):
        if self.cloud_sub is None:
            return
        
        self.get_logger().info('saving point-cloud snapshot...')
        ...
        o3d.io.write_point_cloud('zed_cloud.pcd', o3d_cloud)
        self.get_logger().info('point-cloud snapshot saved as zed_cloud.pcd')

        self.cloud_sub = None  # prevent further snapshots


    # Setting params functionality is causing malfunctioning for some reason so not being used yet

    # def set_param(self, param_name: str, value):
        # self.get_logger().info(f'setting parameter {param_name} to {value}...')
        ...
        # res = future.result()
        # if res.success:
        #     self.get_logger().info(f'parameter {param_name} set successfully to {value}')
        # else:
        #     self.get_logger().error(f'failed to set parameter {param_name} to {value}: {res.reason}')
    ...
    def survey(self) -> bool:
        ...
        if math.hypot(dx, dy) < self.position_tolerance:
            ...
            if self.waypoint_index >= len(self.waypoints):
                self.get_logger().info('survey complete, hovering at last waypoint...')
                ...
                # self.set_param('WPNAV_SPEED', 150)
    ...
    def return_to_home(self) -> bool:
        ...
        # rtl mode wouldn't work without GPS
